...
public abstract class Subsystem {
    ...
    protected void handleUpdate() {
        try {
            ...
            if(idMsg.getText().contains(",")) {
                int index = idMsg.getText().indexOf(",");
                String id1Str = idMsg.getText().substring(0, index);
                String id2Str = idMsg.getText().substring(index+1);
                
                int id1 = Integer.parseInt(id1Str);
                int id2 = Integer.parseInt(id2Str);
                
                sendAllObjects(jpaManager.update(className, id1,id2, params));
            } else {
                int id = Integer.parseInt(idMsg.getText());
                sendAllObjects(jpaManager.update(className, id, params));
            }
        } catch (JMSException ex) {
            Logger.getLogger(Subsystem.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    protected void handleDelete() {
        try {
            ...
            if(idMsg.getText().contains(",")) {
                ...
                deleted = jpaManager.delete(className,id1,id2);
            } else {
                int id = Integer.parseInt(idMsg.getText());
                deleted = jpaManager.delete(className, id);
            }
