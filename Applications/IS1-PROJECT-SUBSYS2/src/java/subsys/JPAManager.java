...
public class JPAManager {
    ...
    public JPAManager() {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("IS1-PROJECT-SUBSYS2PU");
    ...
    public boolean addAudioKategorija(int audioId, int kategorijaId) {
        ...
        try {
            ...
            if (audio == null || kategorija == null) {
                transaction.rollback();
                System.out.println("Audio or kategorija is NULL");
                return false;
            }

            if (audio.getKategorijaList() == null) {
                audio.setKategorijaList(new ArrayList<>());
            }
            if (!audio.getKategorijaList().contains(kategorija)) {
                audio.getKategorijaList().add(kategorija);
            }

            if (kategorija.getAudioSnimakList() == null) {
                kategorija.setAudioSnimakList(new ArrayList<>());
            }
            if (!kategorija.getAudioSnimakList().contains(audio)) {
                kategorija.getAudioSnimakList().add(audio);
            }
    ...
    public boolean removeAudioKategorija(int audioId, int kategorijaId) {
        EntityTransaction transaction = em.getTransaction();
        transaction.begin();
        ...
        try {
            ...
            if (audio.getKategorijaList() != null) {
                audio.getKategorijaList().remove(kategorija);
            }
            if (kategorija.getAudioSnimakList() != null) {
                kategorija.getAudioSnimakList().remove(audio);
            }
